...
export function handleBondCreate(event: BondCreated) : void{
  ...
  DepositAddFrax(transaction, token.id, amount, transaction.timestamp)
  ...
  // deposit.ohmReserve = toDecimal(ohm_contract.balanceOf(Address.fromString(FRAXBOND_CONTRACT1)), 9)
...
export function handleBondRedeem(event: BondRedeemed) : void{
  ...
  redeem.ohmReserve = toDecimal(ohm_contract.balanceOf(Address.fromString(FRAXBOND_CONTRACT1)), 9)
