...
import {  DepositCall, RedeemCall, BondCreated, BondRedeemed, BondPriceChanged, ControlVariableAdjustment} from '../generated/OHMLUSDBond/OHMLUSDBond'
...
import { OHMLUSDBOND_TOKEN, OHM_ERC20_CONTRACT, OHMLUSDBOND_CONTRACT1 } from './utils/Constants'

import { getPairUSD } from './utils/Price'
import {DepositAdd, ControlVariableAdd} from "./utils/YearsDeposit"

/**
    @dev : Handle OHM-LUSD Bond create
*/
export function handleBondCreate(event: BondCreated) : void{
  let transaction = loadOrCreateTransaction(event.transaction, event.block)
  let token = loadOrCreateToken(OHMLUSDBOND_TOKEN)
...
export function handleControlVariableAdjustment(event: ControlVariableAdjustment) : void{
  ...
  variable.save()
  ControlVariableAdd('control', 'OHM-LUSD', toDecimal(event.params.initialBCV, 9), toDecimal(event.params.newBCV, 9), toDecimal(event.params.adjustment, 9), event.params.addition, transaction.timestamp)
}